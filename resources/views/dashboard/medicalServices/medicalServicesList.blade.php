...
@section('content')
    ...
    <div class="form-group col-xl-12 text-center title_header1">
        <strong>{{ __('lang.typical_reports_medical') }}</strong>
    </div>

    <input type="hidden" name="data" id="data_print">

    <div class="card-body">
        <table class="table table-responsive-sm table-bordered table-striped reportsmedicalServices_datatable"
            style="table-layout: fixed;">
            <thead class="text-center">
                <tr class="incoming_tr print_border_tr">
...
@section('javascript')
    <script>
        $(document).ready(function() {
            ...
            var table = $('.reportsmedicalServices_datatable').DataTable({
                ...
                buttons: [{
                    ...
                    title: '<div class="title_header"><strong> {{ __('lang.typical_reports_medical') }} </strong></div>',
                ...
                processing: true,
                ajax: "{{ route('typicalMedicalServices.index') }}",
                columns: [{
                        data: 'id',
                        name: 'id',
                        orderable: false,
                    ...
                    {
                        data: 'no_operating_rooms',
                        name: 'no_operating_rooms',
                        orderable: false
                    },
                ],
                responsive: true,
            ...
            setTimeout(function() {
                var reportsmedicalServices_checkData = $(".reportsmedicalServices_datatable").dataTable()
                    .fnGetData().length;

                if (reportsmedicalServices_checkData === 0) {
                    $('.foundData').hide();
                    $('.NoData').show();
                } else {
                    $('.NoData').hide();
            ...
            $('.btn_pdf').click(function() {
                var data_table = $(".reportsmedicalServices_datatable").dataTable().fnGetData();
                $("#data_print2").val(JSON.stringify(data_table));
                $("#inquiry_form2").attr('action', '{{ route("typicalMedicalServices.pdf") }}')

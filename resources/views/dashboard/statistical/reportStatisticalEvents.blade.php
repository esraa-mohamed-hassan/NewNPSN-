...
@section('content')
    ...
    <style>
        .statistical_events_received_datatable {
            direction: rtl;
        }
    ...
    <div class="form-group col-xl-12 text-center title_header1">
        <strong>{{ __('lang.statistical_events_received') }}</strong>
    </div>

    <input type="hidden" name="data" id="data_print">

    <div class="card-body">
        <table class="table table-responsive-sm table-bordered table-striped statistical_events_received_datatable"
            style="table-layout: fixed;">
            <thead class="text-center">
                <tr class="print_border_tr incoming_tr">
                    <th class="w-50">{{ __('lang.event') }}</th>
                    <th class="w-50">{{ __('lang.statistical_co_event') }}</th>
                </tr>
            </thead>
...
@section('javascript')
    <script>
        $(document).ready(function() {
            ...
            var table = $('.statistical_events_received_datatable').DataTable({
                ...
                buttons: [{
                    ...
                    title: '<div class="title_header"><strong> {{ __('lang.statistical_events_received') }} </strong></div>',
                    customize: function(win) {
                        var last = null;
                        var current = null;
                        var bod = [];

                        var css = '@page { size: A4 landscape;}.table-bordered{border-color:#000}',
                ...
                processing: true,
                ajax: "{{ route('statisticalEvents.index') }}",
                columns: [{
                        data: 'event',
                        name: 'event',
                        orderable: false
            ...
            setTimeout(function() {
                ...
                var statisticalevents_checkData = $(".statistical_events_received_datatable").dataTable()
                    .fnGetData().length;

                if (statisticalevents_checkData === 0) {
                    $('.foundData').hide();
                    $('.NoData').show();
                } else {
                    $('.NoData').hide();
            ...
            $('.btn_pdf').click(function() {
                var data_table = $(".statistical_events_received_datatable").dataTable().fnGetData();
                $("#data_print2").val(JSON.stringify(data_table));
                $("#inquiry_form2").attr('action', '{{ route("statisticalEvents.pdf") }}')
